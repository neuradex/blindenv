...
- **출력 마스킹** — 모든 stdout/stderr를 스캔하여 시크릿 값을 `[BLINDED]`로 치환한 뒤 에이전트에게 전달합니다.
...
blindenv 프록시:  서브프로세스 환경에 실제 값 주입
                         ↓
에이전트 수신:    {"result": "ok", "token": "[BLINDED]"}
```
...
| 2 | **출력 마스킹** | stdout/stderr를 스캔하여 시크릿 값을 `[BLINDED]`로 치환 |
...
blindenv는 세 가지 보안 모드를 제공합니다. 에이전트가 얼마나 볼 수 있는지 선택하세요:

```yaml
# blindenv.yml
mode: block    # blind (기본값) | block | stash
```

| 모드 | 시크릿 파일 접근 | 시크릿 값 | `ls`에 파일 노출? |
|------|-----------------|----------|------------------|
| **`blind`** (기본값) | 읽기 가능 | `[BLINDED]` | 예 |
| **`block`** | 명시적 차단 | 숨김 | 예 |
| **`stash`** | 파일 없음 | 숨김 | **아니오** (물리적 삭제) |

### `blind` — 구조는 보이고, 값은 안 보임 (기본값)

에이전트가 시크릿 파일을 읽을 수 있지만, 모든 시크릿 값이 `[BLINDED]`로 치환됩니다. 변수 이름, 주석, 파일 구조는 그대로 보입니다. 가장 자연스러운 경험 — 차단 메시지도 없고, 에이전트는 중요한 것만 못 봅니다.
...
에이전트가 시크릿 파일을 읽으려 하면 "접근 거부"를 봅니다. 파일이 있다는 것은 알지만 접근할 수 없습니다. 보호 대상을 명확히 알려주고 싶을 때 사용합니다.
...
시크릿은 정상 작동합니다 — 캐시에서 주입과 출력 블라인딩에 사용됩니다. 세션 후 `blindenv cache-restore`로 복원할 수 있습니다.
...
| 필드 | 용도 | 사용 시점 |
|------|------|----------|
| `mode` | 보안 모드: `blind`, `block`, `stash` | 기본 blind 대신 block/stash를 원할 때 |
...
blindenv stash                        시크릿 파일을 캐시로 옮기고 원본 삭제 (stash 모드 전용)
