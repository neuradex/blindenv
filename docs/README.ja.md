...
- **出力マスキング** — すべてのstdout/stderrをスキャンし、シークレット値を`[BLINDED]`に置換してからエージェントに渡します。
...
blindenvプロキシ:  サブプロセス環境に実際の値を注入
                         ↓
エージェント受信:  {"result": "ok", "token": "[BLINDED]"}
```
...
| 2 | **出力マスキング** | stdout/stderrをスキャンし、シークレット値を`[BLINDED]`に置換 |
...
> *「存在自体を知らないことが、最高のセキュリティである。」*

blindenvは3つのセキュリティモードを提供します。エージェントにどこまで見せるかを選べます：

```yaml
# blindenv.yml
mode: block    # blind（デフォルト）| block | stash
```

| モード | シークレットファイルアクセス | シークレット値 | `ls`でファイルが見える？ |
|--------|--------------------------|--------------|----------------------|
| **`blind`**（デフォルト） | 読み取り可能 | `[BLINDED]` | はい |
| **`block`** | 明示的拒否 | 非表示 | はい |
| **`stash`** | ファイルなし | 非表示 | **いいえ**（物理的に削除） |

### `blind` — 構造は見え、値は見えない（デフォルト）

エージェントはシークレットファイルを読めますが、すべてのシークレット値が`[BLINDED]`に置換されます。変数名、コメント、ファイル構造はそのまま見えます。最も自然な体験 — ブロックメッセージもなく、エージェントは重要なものだけ見えません。
...
エージェントがシークレットファイルを読もうとすると「アクセス拒否」が表示されます。ファイルの存在は知っていますがアクセスできません。保護対象を明確に知らせたい場合に使用します。

### `stash` — 完全な隠蔽

最も強力なモード。セッション開始時にシークレットファイルを安全なキャッシュ（`~/.cache/blindenv/`）に移動し、ディスクから物理的に削除します。Bashで`ls`、`find`、`tree`を実行しても何も表示されません。セッション中、ファイルは本当にディスクに存在しません。

シークレットは正常に機能します — キャッシュから注入と出力ブラインディングに使用されます。セッション後に`blindenv cache-restore`で復元できます。
...
| フィールド | 用途 | 使用タイミング |
|-----------|------|--------------|
| `mode` | セキュリティモード: `blind`、`block`、`stash` | デフォルトのblind以外を使用したい場合 |
...
blindenv stash                        シークレットファイルをキャッシュに移動、原本削除（stashモード専用）
blindenv cache-restore                キャッシュからシークレットファイルを復元
