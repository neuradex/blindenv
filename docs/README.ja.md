...
blindenvは、段階的に強化される3つのセキュリティモードを提供します：

```yaml
# blindenv.yml
mode: stealth    # block（デフォルト）| stealth | evacuate
```

| モード | シークレットファイルアクセス | `ls`でファイルが見える？ | 適切な場面 |
|--------|--------------------------|----------------------|----------|
| **`block`** | 明示的拒否 + 出力マスキング | はい | ほとんどのプロジェクト — ブロック時に明確なフィードバック |
| **`stealth`** | ファイルが存在しないように見える | はい（ディスクには存在） | エージェントにシークレットの存在を知られたくない場合 |
| **`evacuate`** | ファイルが存在しないように見える | **いいえ**（物理的に削除） | 最高セキュリティ — `ls`でも何も見えない |

### `block` — 拒否とマスキング（デフォルト）

エージェントがシークレットファイルにアクセスすると「アクセス拒否」メッセージが表示されます。出力は引き続きマスキングされます。エージェントはファイルの存在を知っていますがアクセスできません。ほとんどのプロジェクトで最も安全なデフォルトです。

### `stealth` — ファイルが存在しない

シークレットファイルがまったく存在しないように見えます。Readは「ファイルなし」を返し、検索から静かに除外されます。エージェントにはファイルの存在を知る方法がありません — Bashで`ls`を実行しない限り。
...
シークレットは正常に機能します — キャッシュから注入とマスキングに使用されます。セッション後に`blindenv cache-restore`で復元できます。
...
| フィールド | 用途 | 使用タイミング |
|-----------|------|--------------|
| `mode` | セキュリティモード: `block`、`stealth`、`evacuate` | デフォルトのblock以外を使用したい場合 |
...
blindenv evacuate                     シークレットファイルをディスクから削除（evacuateモード専用）
